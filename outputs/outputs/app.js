...
async function checkHostedConfiguration() {
  if (!isHostedApp()) return;
  try {
    const response = await fetch("/api/config", { cache: "no-store" });
    const config = await response.json();
    const version = document.querySelector(".build-version");
    if (version && config.buildVersion) version.textContent = `Build ${config.buildVersion}`;
    if (!config.googleSheetsConnected) {
      setStatus("Google Sheets is not connected in this Railway service", true);
    }
  } catch {
    setStatus("Unable to check Railway configuration", true);
  }
}
...
function loadRecord(id) {
  ...
  fields.date.value = normalizeDateInput(record.date);
...
document.querySelector("#printBtn").addEventListener("click", () => {
  const hasSupplier = Boolean(fields.supplier.value.trim());
  const hasSubject = Boolean(fields.subject.value.trim());
  const hasLineItem = itemData().some(item => item.description && item.qty > 0);
  if (!hasSupplier || !hasSubject || !hasLineItem) {
    alert("This is a blank LPO. Select the saved LPO from the left panel, or complete the Supplier, Subject, and line-item details before printing.");
    return;
  }
  window.print();
});
